...
import numpy as np
import pyodbc
con = pyodbc.connect("DRIVER={SQL Server};server=localhost;database=Yorumlar")
cursor = con.cursor()
cursor.execute("select olumluSayisi from YorumSay where Id = 1")
rows = [x for x in cursor]
for row in rows:
    olumluSayisi = row[0]
# Veri setinin eklenip başlığının belirlenmesi
column = ['yorum']
df = pd.read_csv('yorumlar.csv', encoding='iso-8859-9', sep='"', error_bad_lines=False)
df.columns = column
df.info()
df.head()
...
df.Positivity.iloc[olumluSayisi:] = 0
...
class duygu_analizi:
    ...
    def classification(self, newcomment):
        print(model.predict(vect.transform([str(newcomment)])))
        sonuc = model.predict(vect.transform([str(newcomment)]))
